                            ...
                            @foreach (config('translatable.locales') as $locale)
                                <div class="tab-pane fade show @if($locale == 'ar') active @endif" id="{{$locale}}" role="tabpanel">
                                    <div class="col form-group">
                                        ...
                                        <input type="text" name="{{ $locale.'[title]' }}" id="{{ $locale . '[title]' }}" placeholder="@lang('general.title')" class="form-control @error("$locale.title" ) is-invalid @enderror" value="{{ $slider->translate($locale)->title }}" required readonly>
                                        @error("$locale.title")
                                    ...
                                    <div class="col form-group">
                                        ...
                                        <textarea name="{{ $locale.'[description]' }}" id="{{ $locale.'[description]'
                                                  ...
                                                  class="form-control @error("$locale.description" ) is-invalid @enderror" cols="30" rows="5" required readonly>{{ $slider->translate($locale)->title }}</textarea>
                                    ...
                                    <div class="col form-group">
                                        ...
                                        <label>@lang('general.' . $locale . '.address')</label>

                                        <input type="text" name="{{ $locale.'[address]' }}" id="{{ $locale . '[address]' }}" placeholder="@lang('general.address')" class="form-control @error("$locale.address" ) is-invalid @enderror" value="{{ $slider->translate($locale)->title }}" readonly>
                                ...
                                <div class="col-lg-9 col-xl-6">
                                    <div class="image-input image-input-outline" id="kt_image_1">
                                        @if(isset($slider->image))
                                            <div class="image-input-wrapper" style="background-image: url({{ $slider->image->path }})"></div>
                                        @else
